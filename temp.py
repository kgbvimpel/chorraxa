...
def save_frame_worker(frame_batch, saved_batches, base_path, folder_range, max_frames):
    ...
    for frame_id, frame in frame_batch:
        ...
        folder_path = base_path / \
            f"{folder_num * folder_range}-{(folder_num + 1) * folder_range - 1}_{ip_address}"
        ...
        try:
            ...
            logging.error("checking saving violation video: %s",
                          traceback.format_exc())  # Logs the traceback
        ...
        while True:
            try:
                ...
                if len(frame_batch) >= batch_size or (frame_id // batch_size > current_batch_start // batch_size):
                    executor.submit(
                        save_frame_worker, frame_batch[:], saved_batches, "./frames", 30, 3000)
        ...
        with threading.Lock():
            if global_video_buffer:
                ...
                if frame_id == last_processed_frame_id:
                    ...
                    # Adjust WAIT_TIME_FOR_NEW_FRAME as needed
                    time.sleep(0.005)
                    continue
                ...
                try:
                    ...
                    processed_frame, detected_objects, tl_color = process_frame(
                        frame, rois)
                    violation_detector.update_tracker(
                        frame_id, detected_objects, ip_address)
                    line_positions = {}
                    ...
                    violation_detector.check_violations(
                        frame_id, tl_color, line_positions, detected_objects)
                    ...
                    for line_info in lines_info:  # Convert stream_id to string to match key
                        if line_info[1] == "roi":
                            ...
                            for i in range(len(points)):
                                cv2.line(processed_frame, tuple(points[i]), tuple(
                                    points[(i + 1) % len(points)]), colors[line_type], 2)
                        else:
                            # Handle other line types (1.1 line, stop_line, red_line) which have two points
                            start_point, end_point, line_type = line_info
                            cv2.line(processed_frame, tuple(start_point),
                                     tuple(end_point), colors[line_type], 2)
                    ...
                    for obj in detected_objects:
                        ...
                        cv2.rectangle(
                            processed_frame, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (0, 255, 0), 2)

                        # Put text (ID and class) near the bounding box
                        text = f"ID: {object_id}, Class: {object_class}"
                        cv2.putText(
                            processed_frame, text, (bbox[0], bbox[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
...
def main():
    ...
    capture_thread = threading.Thread(target=frame_capture_loop, args=(
        rtsp_url, global_video_buffer, MAX_FRAMES_IN_BUFFER))
    processing_thread = threading.Thread(target=main_loop, args=(
        global_video_buffer, lines_info, ip_address))
